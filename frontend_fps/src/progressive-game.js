...
// ─── Progressive Server Integration (Stage-based) ────────────────────────
const query = new URLSearchParams(window.location.search);
const integrationStage = Number(query.get('stage') || '0');
const serverGameId = query.get('game') || '';
let serverObs = null;
const serverEntityMeshes = {};

const stageBadge = document.createElement('div');
stageBadge.style.position = 'fixed';
stageBadge.style.top = '10px';
stageBadge.style.left = '50%';
stageBadge.style.transform = 'translateX(-50%)';
stageBadge.style.padding = '6px 10px';
stageBadge.style.fontFamily = "'Segoe UI', system-ui, sans-serif";
stageBadge.style.fontSize = '12px';
stageBadge.style.color = '#d7dde8';
stageBadge.style.background = 'rgba(10,14,24,0.78)';
stageBadge.style.border = '1px solid rgba(120,146,192,0.45)';
stageBadge.style.borderRadius = '8px';
stageBadge.style.zIndex = '999';
stageBadge.style.pointerEvents = 'none';
stageBadge.textContent = `progressive stage=${integrationStage}`;
document.body.appendChild(stageBadge);

function serverRotationToQuaternion(rot) {
...
function serverGeometry(entity) {
  const s = entity.size || [1, 1, 1];
  const shape = String(entity.shape || '').toLowerCase();
  if (shape === 'ball') return new THREE.SphereGeometry(Math.max(0.2, s[0] * 0.5), 12, 10);
  if (shape === 'cylinder') {
    const r = Math.max(0.2, Math.max(s[0], s[2]) * 0.5);
    return new THREE.CylinderGeometry(r, r, Math.max(0.2, s[1]), 12);
  }
  return new THREE.BoxGeometry(Math.max(0.2, s[0]), Math.max(0.2, s[1]), Math.max(0.2, s[2]));
}

function syncServerEntityOverlay(obs) {
  if (integrationStage < 2 || !obs || !Array.isArray(obs.entities)) return;

  const seen = {};
  for (const entity of obs.entities) {
    if (!entity || typeof entity.id !== 'number') continue;
    seen[entity.id] = true;
    let mesh = serverEntityMeshes[entity.id];
    if (!mesh) {
      mesh = new THREE.Mesh(
        serverGeometry(entity),
      ...
      scene.add(mesh);
      serverEntityMeshes[entity.id] = mesh;
    }
    ...
    if (entity.rotation) {
      mesh.quaternion.copy(serverRotationToQuaternion(entity.rotation));
    }
  }

  for (const id of Object.keys(serverEntityMeshes)) {
    const numId = Number(id);
    if (seen[numId]) continue;
    scene.remove(serverEntityMeshes[numId]);
    serverEntityMeshes[numId].geometry.dispose();
    serverEntityMeshes[numId].material.dispose();
    delete serverEntityMeshes[numId];
  }
}

function connectServerBridge() {
  if (integrationStage < 1) {
    stageBadge.textContent = 'progressive stage=0 (reference only)';
    return;
  }
  if (!serverGameId) {
    stageBadge.textContent = 'progressive stage>=1 requires ?game=<uuid>';
    return;
  }

  const proto = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
  const ws = new WebSocket(`${proto}//${window.location.host}/api/v1/games/${serverGameId}/spectate/ws`);
  ws.binaryType = 'arraybuffer';
  stageBadge.textContent = `progressive stage=${integrationStage} connecting...`;
  ...
  ws.onmessage = (event) => {
    try {
      ...
      if (parsed && parsed.error) {
        stageBadge.textContent = `server error: ${parsed.error}`;
        return;
      }
      serverObs = parsed;
      const n = Array.isArray(parsed.entities) ? parsed.entities.length : 0;
      stageBadge.textContent = `progressive stage=${integrationStage} entities=${n}`;
      syncServerEntityOverlay(parsed);
    } catch {
      stageBadge.textContent = `progressive stage=${integrationStage} parse error`;
    }
  };

  ws.onclose = () => {
    stageBadge.textContent = `progressive stage=${integrationStage} disconnected; reconnecting...`;
    window.setTimeout(connectServerBridge, 1200);
  };
  ws.onerror = () => {
    stageBadge.textContent = `progressive stage=${integrationStage} ws error`;
  };
}
